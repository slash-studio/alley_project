...
<div id="top_block">
  ...
  <form action="/admin/courses" method="post" class="item_edit" id="item{$course.courses_id}">
    ...
    <ul class="imgs">
      ...
      <li><a href="/scripts/uploads/{$photo}_s.jpg" class="block"><img src="/scripts/uploads/{$photo}_s.jpg" /></a><button class="x" data="{$photo}">x</button><input type="radio" name="make_main" value="{$photo}" /></li>
      {/foreach}
    </ul>
  </form>
  {/foreach}
