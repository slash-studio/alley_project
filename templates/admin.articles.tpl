...
<div id="top_block">
  ...
  {if $article_level==2}
    ...
    <form action="/admin/articles/{$year}/{$month}" method="post" class="item_edit" id="item{$article.news_id}">
      ...
      <button class="upload" data='{literal}{{/literal}"upload_type":"articles", "item_id":"{$article.articles_id}", "count":"1", "width":"", "height":"", "sizes":"s,b"{literal}}{/literal}'>Загрузить фото</button>
      <ul class="imgs">
